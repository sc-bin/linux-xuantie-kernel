							...
							unsigned long parent_rate)
{
	...
	switch (k230_composite->rate_calc_method) {
		...
		case 0: {
			...
			reg_div = (readl(k230_composite->rate_reg) >>
		...
		case 2: {
			if ((k230_composite->rate_reg_1) == NULL) {
				spin_lock_irqsave(k230_composite->composite_spinlock,
						flags);
				reg_mul = (readl(k230_composite->rate_reg) >>
					k230_composite->rate_mul_shift) &
					k230_composite->rate_mul_mask;
				reg_div = (readl(k230_composite->rate_reg) >>
					k230_composite->rate_div_shift) &
					k230_composite->rate_div_mask;
				spin_unlock_irqrestore(
					k230_composite->composite_spinlock, flags);
			} else {
				spin_lock_irqsave(k230_composite->composite_spinlock,
						flags);
				reg_mul = (readl(k230_composite->rate_reg_1) >>
					k230_composite->rate_mul_shift_1) &
					k230_composite->rate_mul_mask_1;
				reg_div = (readl(k230_composite->rate_reg) >>
					k230_composite->rate_div_shift) &
					k230_composite->rate_div_mask;
				spin_unlock_irqrestore(
		...
		default: {
			pr_err("[K230_CLK]:clk %s recalc rate error.",
				clk_hw_get_name(hw));
			return 0;
		}
	}
...
static int k230_clk_find_approximate(struct k230_clk_composite *k230_composite,
						...
						uint32_t method, unsigned long rate,
						unsigned long parent_rate, uint32_t *div,
						uint32_t *mul)
{
	long abs_min;
	long abs_current;
	...
	uint32_t codec_clk[9] = { 2048000,  3072000,  4096000,
					6144000,  8192000,  11289600,
					12288000, 24576000, 49152000 };
	uint32_t codec_div[9][2] = { { 3125, 16 }, { 3125, 24 },
						{ 3125, 32 }, { 3125, 48 },
						{ 3125, 64 }, { 15625, 441 },
						{ 3125, 96 }, { 3125, 192 },
						{ 3125, 384 } };
	...
	uint32_t pdm_clk[20] = {
		...
		768000,	 1024000,  1411200,  1536000,  2048000,
		2822400, 3072000,  4096000,  5644800,  6144000,
		8192000, 11289600, 12288000, 24576000, 49152000
	};
	uint32_t pdm_div[20][2] = {
		{ 3125, 1 },	{ 6250, 3 },	{ 3125, 2 },	{ 3125, 3 },
		{ 3125, 4 },	{ 3125, 6 },	{ 3125, 8 },	{ 125000, 441 },
		{ 3125, 12 },	{ 3125, 16 },	{ 62500, 441 }, { 3125, 24 },
		{ 3125, 32 },	{ 31250, 441 }, { 3125, 48 },	{ 3125, 64 },
		{ 15625, 441 }, { 3125, 96 },	{ 3125, 192 },	{ 3125, 384 }
	};
	...
	switch (method) {
		...
		/* mul and div can be changeable. */
		case 2: {
			if (K230_CLK_CODEC_ADC_MCLKDIV_OFFSET ==
					k230_composite->rate_reg -
						k230_composite->base_reg ||
			...
			} else if (K230_CLK_AUDIO_CLKDIV_OFFSET ==
					k230_composite->rate_reg -
						k230_composite->base_reg ||
				K230_CLK_PDM_CLKDIV_OFFSET ==
					k230_composite->rate_reg -
						k230_composite->base_reg) {
				for (j = 0; j < 20; j++) {
					if ((rate - pdm_clk[j]) == 0) {
		...
		default: {
			pr_err("[canaan_clk_err]:k230_clk_find_approximate_ method error!\n");
			return -1;
		}
	}
#ifdef K230_CLK_DEBUG
					   ...
					   unsigned long parent_rate)
{
	...
	if ((k230_composite->rate_reg_1) == NULL) {
		...
		if (k230_composite->rate_calc_method == 2) {
			reg &= ~((k230_composite->rate_mul_mask)
				 << (k230_composite->rate_mul_shift));
		}
		...
		reg &= ~((k230_composite->rate_div_mask)
			 << (k230_composite->rate_div_shift));
		reg_1 &= ~((k230_composite->rate_mul_mask_1)
			   << (k230_composite->rate_mul_shift_1));
	...
	pr_info("[K230_CLK]:clk_set_rate %s rate %d, parent_rate %d",
		clk_hw_get_name(hw), (int32_t)rate, (uint32_t)parent_rate);
#endif
	return 0;
}

static long k230_clk_composite_round_rate(struct clk_hw *hw, unsigned long rate,
					  unsigned long *parent_rate)
{
	return rate;
}
...
static int k230_clk_composite_enable(struct clk_hw *hw)
{
	...
	pr_info("[K230_CLK]:enable clk %s addr 0x%08x reg_value 0x%0x",
		clk_hw_get_name(hw), (int)(long)k230_composite->gate_reg,
...
#ifdef K230_CLK_DEBUG
	pr_info("set parent %s clk mux addr 0x%08x value 0x%08x shift %d mask 0x%08x return %d",
		clk_hw_get_name(hw), (int)(long)k230_composite->mux_reg,
		(int)composite_mux_value, k230_composite->mux_shift,
		k230_composite->mux_mask,
		(composite_mux_value >> k230_composite->mux_shift) &
			k230_composite->mux_mask);
#endif
	return 0;
}
...
#ifdef K230_CLK_DEBUG
	pr_info("get parent %s clk mux addr 0x%08x value 0x%08x shift %d mask 0x%08x return %d",
		clk_hw_get_name(hw), (int)(long)k230_composite->mux_reg,
		(int)composite_mux_value, k230_composite->mux_shift,
		k230_composite->mux_mask,
		(composite_mux_value >> k230_composite->mux_shift) &
...
#ifdef K230_CLK_DEBUG
		pr_info("[K230_CLK]:%s hardware auto enable, set software enable!",
			clk_hw_get_name(hw));
#endif
...
static struct clk *_of_cannan_k230_clk_composite_setup(struct device_node *node)
{
	...
	k230_clk_composite =
		kzalloc(sizeof(struct k230_clk_composite), GFP_KERNEL);
	if (k230_clk_composite == NULL) {
		pr_err("[canaan_clk_err]:Could not kzalloc sruct k230_clk_composite.\n");
		goto out_unmap;
	}
	k230_clk_composite->base_reg = base_reg;

	if (of_property_read_u32(node, "clk-parent-reg-offset",
				 &composite_reg_offset))
		mux = false;
	else {
		mux = true;
		k230_clk_composite->mux_reg =
	...
	if ((of_property_read_u32(node, "clk-rate-reg-offset",
				  &composite_reg_offset)))
		rate = false;
	else {
		rate = true;
		k230_clk_composite->rate_reg =
			(void __iomem *)((uint64_t)base_reg +
					 (uint64_t)composite_reg_offset);
	...
	/* parse the read-only */
	if (of_property_read_u32(node, "read-only",
	...
	parent_name =
		kcalloc(init.num_parents, sizeof(const char *), GFP_KERNEL);
	if (!parent_name)
		goto free_k230_clk_composite;
	init.num_parents =
...
struct k230_clk_pll {
	struct clk_hw hw;
	struct clk_lookup *
		cl; /* clock lookup by clk_get() api, if don't regist, please use __clk_lookup*/
...
/* query pll lock status */
	if (!(readl(k230_pll->pll_reg_lock) &
			(1 << k230_pll->pll_lock_status_bit))) {
		spin_unlock_irqrestore(k230_pll->pll_spinlock, flags);
...
/* pll output freqency = osc24m * (fb_div+1) / (ref_div+1) / (out_div+1) */
	...
	fb_div = ((pll_divide_value >>
			k230_pll->pll_fb_div_shift) &
			k230_pll->pll_fb_div_mask) +
			1;
	ref_div = ((pll_divide_value >>
			k230_pll->pll_ref_div_shift) &
			k230_pll->pll_ref_div_mask) +
			1;
	out_div = ((pll_divide_value >>
			k230_pll->pll_out_div_shift) &
...
static struct clk *_of_canaan_k230_pll_clk_setup(struct device_node *node)
{
	...
	if ((of_property_read_u32(node, "clk-divide-reg-offset",
		...
		k230_clk_pll->pll_reg_divide =
			(void __iomem *)((uint64_t)base_reg +
					 (uint64_t)pll_reg_offset);
	}
...
static struct dentry *canaan_clk_debug_dir = (struct dentry *)NULL;
...
DEFINE_SIMPLE_ATTRIBUTE(composite_enable_fops,
			k230_debugfs_composite_enable_get,
			k230_debugfs_composite_enable_set, "%llu\n");
...
DEFINE_SIMPLE_ATTRIBUTE(composite_rate_fops, k230_debugfs_composite_rate_get,
			k230_debugfs_composite_rate_set, "%llu\n");
...
DEFINE_SIMPLE_ATTRIBUTE(composite_mux_fops, k230_debugfs_composite_mux_get,
			k230_debugfs_composite_mux_set, "%llu\n");
...
static int composite_show_register_show(struct seq_file *s, void *data)
{
	...
	if (k230_composite->gate_reg != NULL) {
		...
		seq_puts(s, "   does not support gate!\n");
	seq_puts(s, "--------------\n\n");

	seq_puts(s, "2. mux info:\n");
	seq_puts(s, "--------------\n");
...
static int k230_debugfs_clk_composite_init(struct clk_hw *hw)
{
	...
	if (canaan_clk_debug_dir == NULL) {
		canaan_clk_debug_dir = debugfs_create_dir("canaan_clk", NULL);
		if (canaan_clk_debug_dir == NULL)
			return -ENOMEM;
	}

	k230_composite->debugfs =
		debugfs_create_dir(clk_hw_get_name(hw), canaan_clk_debug_dir);

	if (!k230_composite->debugfs)
		return -ENOMEM;

	if (!debugfs_create_file("enable", 0600, k230_composite->debugfs, hw,
				 &composite_enable_fops))
		return -ENOMEM;

	if (!debugfs_create_file("rate", 0600, k230_composite->debugfs, hw,
				 &composite_rate_fops))
		return -ENOMEM;

	if (!debugfs_create_file("mux", 0600, k230_composite->debugfs, hw,
				 &composite_mux_fops))
		return -ENOMEM;

	if (!debugfs_create_file("reg_info", 0444, k230_composite->debugfs, hw,
				 &composite_show_register_fops))
		return -ENOMEM;
...
DEFINE_SIMPLE_ATTRIBUTE(pll_enable_fops, k230_debugfs_pll_enable_get,
			k230_debugfs_pll_enable_set, "%llu\n");
...
DEFINE_SIMPLE_ATTRIBUTE(pll_rate_fops, k230_debugfs_pll_rate_get,
			k230_debugfs_pll_rate_set, "%llu\n");
...
static int k230_debugfs_clk_pll_init(struct clk_hw *hw)
{
	...
	if (!debugfs_create_file("enable", 0600, k230_pll->debugfs, hw,
				 &pll_enable_fops))
		return -ENOMEM;

	if (!debugfs_create_file("rate", 0600, k230_pll->debugfs, hw,
				 &pll_rate_fops))
		return -ENOMEM;

	if (!debugfs_create_file("reg_info", 0444, k230_pll->debugfs, hw,
				 &pll_show_register_fops))
		return -ENOMEM;
