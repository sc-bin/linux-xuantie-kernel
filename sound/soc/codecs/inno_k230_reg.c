...
static struct acodec_sound_values g_snd_default_values;
static struct acodec_sound_values g_snd_current_values;
...
//powerup 配置中sel_vref每次配置都要延迟20ms,adc的step1/step2，
//dac的step1延迟10ms，其他延迟1ms

static struct audio_codec_reg_s *audio_codec_reg;
//tatic rt_thread_t                g_check_hp_thread = 0;
static bool g_adc_left_mute;
...
static int _reset_snd_values(struct acodec_sound_values *snd_values)
{
	...
	snd_values->adc_volumel = 0xe1;
	snd_values->adc_volumer = 0xf0;
...
// reg0.reg_0.digcore_bstn = 1;
	...
	// 1.Configure the register POP_CTRL_DACL[1:0] 0x29[5:4] to 2’b01,
	...
	// 1.Configure the register POP_CTRL_DACR[1:0] 0x2c[5:4] to 2’b01,
	...
	// 5.Change the register SEL_VREF[7:0] reg0x21[7:0] :
	// configure the reg0x21[7:0] to 7’b1111_1111 directly.
	...
	// 7.Configure the register SEL_VREF[7:0] reg0x21[7:0] to the
	//appropriate value(except 7’b0000_00000) for reducing power
	reg21.reg_data = readl(&audio_codec_reg->reg_21);
...
void audio_codec_adc_init(enum k_i2s_work_mode mode, unsigned int i2s_ws)
{
	...
	// 1.Configure the register MUTE_MICL 0x24[7] to 1,
	// to end the mute station of the left ADC channel.
	reg24.reg_data = readl(&audio_codec_reg->reg_24);
	reg24.reg_24.mute_micl = 1;
	writel(reg24.reg_data, &audio_codec_reg->reg_24);
	msleep(AUDIO_REG_CONFIG_LONG2_DELAY);

	// 1.Configure the register MUTE_MICR 0x27[7] to 1,
	// to end the mute station of the left ADC channel
	reg27.reg_data = readl(&audio_codec_reg->reg_27);
	...
	// 2.Configure the register EN_IBIAS_ADC 0x20[5] to 1,
	...
	// 3.Configure the register EN_BUF_ADCL 0x23[7] to 1,
	...
	// 3.Configure the register EN_BUF_ADCR 0x26[7] to 1,
	...
	// 4.Configure the register EN_MICL 0x23[6] to 1,
	...
	// 4.Configure the register EN_MICR 0x26[6] to 1,
	...
	// 5.Configure the register EN_ALCL 0x23[5] to 1,
	...
	// 5.Configure the register EN_ALCR 0x26[5] to 1,
	...
	// 6.Configure the register EN_CLK_ADCL 0x23[4] to 1,
	...
	// 6.Configure the register EN_CLK_ADCR 0x26[4] to 1,
	...
	// 7.Configure the register EN_ADCL 0x23[3] to 1,
	...
	// 7.Configure the register EN_ADCR 0x26[3] to 1,
	...
	// i2s rx fmt
	reg4.reg_data = readl(&audio_codec_reg->reg_04);
	if (mode == K_STANDARD_MODE)
		reg4.reg_4.i2s_rx_fmt = 2;
	else if (mode == K_RIGHT_JUSTIFYING_MODE)
		reg4.reg_4.i2s_rx_fmt = 0;
	else if (mode == K_LEFT_JUSTIFYING_MODE)
	...
	// 8.Configure the register INITIAL_ADCL 0x23[2] to 1,
	...
	// 8.Configure the register INITIAL_ADCR 0x26[2] to 1,
	...
	// 9.Configure the register INITIAL_ALCL 0x23[1] to 1,
	...
	// 9.Configure the register INITIAL_ALCR 0x26[1] to 1,
	...
	// 10.Configure the register INITIAL_MICL 0x23[0] to 1,
	// to end the initialization of theleft MIC module.
	reg23.reg_data = readl(&audio_codec_reg->reg_23);
	reg23.reg_23.initial_micl = 1;
	writel(reg23.reg_data, &audio_codec_reg->reg_23);
	msleep(AUDIO_REG_CONFIG_NORMAL_DELAY);

	// 10.Configure the register INITIAL_MICR 0x26[0] to 1,
    ...
    //11.Configure the register MUTE_MICL 0x24[7] to 1,
	// to end the mute station of the ADC left channel.
    // reg24.reg_data = readl(&audio_codec_reg->reg_24);
    // reg24.reg_24.mute_micl = 1;
    // writel(reg24.reg_data,&audio_codec_reg->reg_24);

    //11.Configure the register MUTE_MICR 0x27[7] to 1,
	...
	// 14.Configure the register EN_ZeroDET_ADCL 0x25[1] to 1,
	...
	// 14.Configure the register EN_ZeroDET_ADCR 0x28[1] to 1,
	...
	// {
	//     g_check_hp_thread = rt_thread_create("acodec_hp", _check_adc_hp_thread,
	//	RT_NULL, 1024*10, 10,10);
...
void audio_codec_dac_init(enum k_i2s_work_mode mode, unsigned int i2s_ws)
{
	...
	// 2.Configure the register EN_BUF_DACL 0x29[6] to 1,
	...
	// 2.Configure the register EN_BUF_DACR 0x2c[6] to 1,
	// to enable the reference voltage buffer of the DAC right channel.
	reg2c.reg_data = readl(&audio_codec_reg->reg_2c);
	reg2c.reg_2c.en_buf_dacr = 1;
	writel(reg2c.reg_data, &audio_codec_reg->reg_2c);
	msleep(AUDIO_REG_CONFIG_NORMAL_DELAY);

	// 3.Configure the register POP_CTRL_DACL<1:0> 0x29[5:4] to 2’b10,
	...
	// 3.Configure the register POP_CTRL_DACR<1:0> 0x2c[5:4] to 2’b10,
	...
	// 4.Configure the register EN_HPOUTL 0x2a[5] to 1,
	...
	// 4.Configure the register EN_HPOUTR 0x2d[5] to 1,
	...
	// 5.Configure the register INITIAL_HPOUTL 0x2a[4] to 1,
	...
	// 5.Configure the register INITIAL_HPOUTR 0x2d[4] to 1,
	// to end the initialization of the HPDRV module in the DAC right channel.
	reg2d.reg_data = readl(&audio_codec_reg->reg_2d);
	reg2d.reg_2d.initial_hpoutr = 1;
	writel(reg2d.reg_data, &audio_codec_reg->reg_2d);
	msleep(AUDIO_REG_CONFIG_NORMAL_DELAY);

	// 6.Configure the register EN_VREF_DACL 0x29[3] to 1,
	...
	// 6.Configure the register EN_VREF_DACR 0x2c[3] to 1,
	...
	// 7.Configure the register EN_CLK_DACL 0x29[2] to 1,
	...
	// 7.Configure the register EN_CLK_DACR 0x2c[2] to 1,
	...
	// 8.Configure the register EN_DACL 0x29[1] to 1,
	...
	// 8.Configure the register EN_DACR 0x2c[1] to 1,
	// to enable the DACR module.
	reg2c.reg_data = readl(&audio_codec_reg->reg_2c);
	...
	// 9.Configure the register INITIAL_DACL 0x29[0] to 1,
	...
	// 9.Configure the register INITIAL_DACR 0x2c[0] to 1,
	...
	// 10.Configure the register MUTE_HPOUTL 0x29[7] to 1,
	...
	// 10.Configure the register MUTE_HPOUTR 0x2c[7] to 1,
...
int audio_codec_adc_set_micl_gain(int gain)
{
	...
	//printk("%s gain:%d\n",__func__, gain);
	if (convert_adc_gain(gain, &reg_val) != 0)
		return -1;
...
int audio_codec_dac_set_hpoutl_gain(int gain)
{
	...
	if (convert_dac_gain(gain, &reg_val) != 0)
		return -1;
